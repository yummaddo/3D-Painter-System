...
namespace Game.Paint3D.Painting.Required
{
...
#if UNITY_EDITOR
	[CustomEditor(typeof(CwMeshFixer))]
	public class CwMeshFixer_Editor : CwEditor
	{
		...
		{
			CwMeshFixer tgt; CwMeshFixer[] tgts; GetTargets(out tgt, out tgts);
		...
		public static void CreateMeshFixerAsset(Mesh mesh, string path)
		{
			...
			if (mesh != null)
			{
				...
				if (modelImporter != null)
				{
					...
					name = "Mesh Fixer (" + System.IO.Path.GetFileNameWithoutExtension(modelImporter.assetPath) + ")";
				}
				else
				{
					asset.AddMesh(mesh);
