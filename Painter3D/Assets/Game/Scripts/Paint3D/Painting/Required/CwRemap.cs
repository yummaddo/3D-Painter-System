...
namespace Game.Paint3D.Painting.Required
{
	...
	public static class CwEditorRemap
	{
		...
		public static Texture2D Remap(Texture2D sourceTexture, Mesh oldMesh, Mesh newMesh, int newSize)
		{
			...
			var canvas       = Game.Paint3D.Core.CwCommon.GetRenderTexture(desc);
			...
			var generatedTexture = Game.Paint3D.Core.CwCommon.GetReadableCopy(canvas);

			Game.Paint3D.Core.CwCommon.ReleaseRenderTexture(canvas);
