...
namespace Game.Paint3D.Painting.Required
{
	...
	[AddComponentMenu(Game.Paint3D.Core.CwCommon.ComponentHitMenuPrefix + "Paint Decal")]
	public class CwPaintDecal : MonoBehaviour, IHitPoint, IHitLine, IHitTriangle, IHitQuad, IHitCoord
	{
		...
		/// <summary>This allows you to mask all paint that would otherwise apply to surfaces behind the specified depth mask rendered by a camera.</summary>
		public CwRenderDepth DepthMask { set { depthMask = value; } get { return depthMask; } } [SerializeField] private CwRenderDepth depthMask;
		...
		public void HandleHitPoint(bool preview, int priority, float pressure, int seed, Vector3 position, Quaternion rotation)
		{
			...
			var worldRadius   = Game.Paint3D.Core.CwCommon.GetRadius(worldSize);
		...
		public void HandleHitLine(bool preview, int priority, float pressure, int seed, Vector3 position, Vector3 endPosition, Quaternion rotation, bool clip)
		{
			...
			var worldSize     = HandleHitCommon(preview, pressure, seed, rotation);
			var worldRadius   = Game.Paint3D.Core.CwCommon.GetRadius(worldSize, position, endPosition);
			var worldPosition = Game.Paint3D.Core.CwCommon.GetPosition(position, endPosition);
		...
		public void HandleHitTriangle(bool preview, int priority, float pressure, int seed, Vector3 positionA, Vector3 positionB, Vector3 positionC, Quaternion rotation)
		{
			...
			var worldSize     = HandleHitCommon(preview, pressure, seed, rotation);
			var worldRadius   = Game.Paint3D.Core.CwCommon.GetRadius(worldSize, positionA, positionB, positionC);
			var worldPosition = Game.Paint3D.Core.CwCommon.GetPosition(positionA, positionB, positionC);
		...
		public void HandleHitQuad(bool preview, int priority, float pressure, int seed, Vector3 position, Vector3 endPosition, Vector3 position2, Vector3 endPosition2, Quaternion rotation, bool clip)
		{
			...
			var worldSize     = HandleHitCommon(preview, pressure, seed, rotation);
			var worldRadius   = Game.Paint3D.Core.CwCommon.GetRadius(worldSize, position, endPosition, position2, endPosition2);
			var worldPosition = Game.Paint3D.Core.CwCommon.GetPosition(position, endPosition, position2, endPosition2);
		...
		private Vector3 HandleHitCommon(bool preview, float pressure, int seed, Quaternion rotation)
		{
			...
			var finalAspect = Game.Paint3D.Core.CwCommon.GetAspect(shape, finalTexture);
			var finalSize   = Game.Paint3D.Core.CwCommon.ScaleAspect(finalScale * finalRadius, finalAspect);
		...
		private void HandleMaskCommon(Vector3 worldPosition)
		{
			...
			CwCommandDecal.Instance.DepthMask = depthMask;
		}
	...
	[CustomEditor(typeof(CwPaintDecal))]
	public class CwPaintDecal_Editor : CwEditor
	{
		protected override void OnInspector()
		{
			CwPaintDecal tgt; CwPaintDecal[] tgts; GetTargets(out tgt, out tgts);
